...
extern ECCPUnMarMsg_t sEccpMsg;
...
)
{
    ...
    eccpMgr.mgrTxQueue = 0UL;
    eccpMgr.mgrLock = 0UL;
    eccpMgr.mgrRxEvent = 0UL;
    eccpMgr.mgrTxEvent = 0UL;
    eccpMgr.mgrRxDoneEvent = 0UL;
...
)
{
    ...
    if (eccpIsInit == 1UL)
    {
        ...
        {
            eccpDevSlot[ucFeature].devQueue  = 0UL;
            eccpDevSlot[ucFeature].devLock   = 0UL;
...
)
{
    ...
    if ((eccpDev != NULL) && (eccpDev->devInUsed == 1UL))
    {
        ...
        if (ECCP_SUCCESS == (ret = ECCP_MakeFrame(ucTarget, &eccpDev->devTxMsg, eccpDev->devDevId, puiCmd2, pucBuff, uiLength)))
        {
            ...
            #else
            if (ECCP_SUCCESS != ECCP_DeployTxData(&eccpDev->devTxMsg, &rxMsgIntxTask, sizeof(ECCPMsg_t)))
            #endif
            {
...
)
{
    ...
    mcu_printf("%s Check Rx message\n", __func__);

    while( TRUE )
    {
        if( eccpMgr.mgrRxEvent == 1UL )
        {
            mcu_printf("%s Receive Rx message\n", __func__);

            SAL_MemSet(&sEccpMsg, 0x00, sizeof(ECCPUnMarMsg_t));

            flag = 1; //temporary modification
            ECCP_DoRx(psRxMsg->msgPtr, flag);

            if(eccpDevSlot[ECCP_UPDATE_APP].devQueue == 1)
            {
                eccpDevSlot[ECCP_UPDATE_APP].devQueue = 0;
...
)
{
    if (ECCP_CHECK_TIMEOUT(flag))
    {
        ...
        {
            while( eccpMgr.mgrRxDoneEvent == 0UL) { ; }
            eccpMgr.mgrRxDoneEvent = 0UL;
...

{
    ...
    if (ECCP_CHECK_START_FRAME(pucBuff) && ECCP_CHECK_VCP_FRAME(pucBuff))
    {
        ...
        if (ECCP_CHECK_VALID_DEVICE(dev))
        {
            ...
            {
                SAL_MemCopy(&sEccpMsg, &rxMsg, sizeof(ECCPUnMarMsg_t));

                eccpDevSlot[dev].devQueue = 1U;
            }
...
)
{
  ...
  //          mcu_printf("%s event %d\n", __func__, iEvent);
        ...
        {
            eccpMgr.mgrTxEvent = 1U;
        }
        else if (iEvent == GPSB_EVENT_COMPLETE)
        {
            eccpMgr.mgrRxDoneEvent = 1U;
        }
    }
}
