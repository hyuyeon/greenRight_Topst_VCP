...
#define GDMA_CON_BASE(x)                ((uint32)MCU_BSP_GDMA_BASE + ((x) * 0x10000UL))
...
#define GDMA_CH_SRC_ADDR(x)             (((uint32)0x100UL + ((x) * 0x20UL)))
#define GDMA_CH_DST_ADDR(x)             (((uint32)0x104UL + ((x) * 0x20UL)))
#define GDMA_CH_LLI(x)                  (((uint32)0x108UL + ((x) * 0x20UL)))
#define GDMA_CH_CON(x)                  (((uint32)0x10CUL + ((x) * 0x20UL)))
#define GDMA_CH_CONFIG(x)               (((uint32)0x110UL + ((x) * 0x20UL)))

#define GDMA_CH0_FLAG                   ((uint32)1UL << 0UL)
#define GDMA_CH1_FLAG                   ((uint32)1UL << 1UL)
...
)
{
    ...
    {
        (void)SAL_DbgReportError(SAL_DRVID_GDMA, 0UL, (uint32)SAL_ERR_INVALID_PARAMETER, __FUNCTION__);
    }
}
...
{
    sint32  ret = (sint32)SAL_RET_SUCCESS;
    sint32  iClkBusId;

    if ( ucCon < GDMA_CON_MAX)
    {
        iClkBusId   = (sint32)CLOCK_IOBUS_DMA_CON0 + (sint32)ucCon;
...
{
    uint32              uiIntStatus;
    uint32              uiTCIntStatus;
    uint32              uiErrIntStatus;
    uint32              uiAddr;
    ...
    if (pArg != NULL_PTR)
    {
        ...
        for(uiIndex = 0; uiIndex < GDMA_CH_MAX; uiIndex++)
        {
            ...
            if ((uiIntStatus & ((uint32)1UL << uiIndex)) != 0UL)
            {
                // check terminal count interrupt status.
                if ((uiTCIntStatus & ((uint32)1UL << uiIndex)) != 0UL)
                {
                    ...
                    if (dmacon != NULL_PTR) //Codesonar : Null Test After Dereference
                    {
                        ...
                        SAL_WriteReg(((uint32)1UL << uiIndex), uiAddr);
                ...
                if ((uiErrIntStatus & ((uint32)1UL << uiIndex)) != 0UL)
                {
                    dmacon = (GDMAInformation_t *)pConTable->cCh[uiIndex];
                    if (dmacon != NULL_PTR) //Codesonar : Null Test After Dereference
                    {
...
)
{
    ...
    if (sDmacon != NULL_PTR)
    {
        ...
        {
            uiValue = SAL_ReadReg(uiAddr);
            if((uiValue & ((uint32)1UL << sDmacon->iCh)) == 0UL) // Currently the channel is disabled(Transfer Completed)
            {
                break;
            }
            BSP_NOP_DELAY();
...
)
{
    ...
    if (sDmacon != NULL_PTR)
    {
        ...
        uiAddr = GDMA_CON_BASE(sDmacon->iCon) + GDMA_ECR; // Enabled Channel Register
        uiValue = SAL_ReadReg(uiAddr);
        if((uiValue & ((uint32)1UL << sDmacon->iCh)) != 0UL) // Currently the channel is enabled
        {
            uiAddr  = GDMA_CON_BASE(sDmacon->iCon) + GDMA_CH_CONFIG(sDmacon->iCh);
            uiValue = SAL_ReadReg(uiAddr);
...
)
{
    ...
    if (sDmacon != NULL_PTR)
    {
        iClkId = (sint32)CLOCK_IOBUS_DMA_CON0 + (sint32)sDmacon->iCon;
        ...
        if ((SAL_ReadReg(uiAddr) & 0x1UL) == 0UL)
        {
            ...
            (void)GIC_IntVectSet((uint32)GIC_DMA0 + sDmacon->iCon, uiIntPrio, GIC_INT_TYPE_LEVEL_HIGH, (GICIsrFunc)&GDMA_ISR, (void*)&gdma_con_table[sDmacon->iCon]);
            (void)GIC_IntSrcEn((uint32)GIC_DMA0 + sDmacon->iCon);
